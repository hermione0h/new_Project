...
nfft = 512;
overlap = 360;
%path1 = '/data/liyuy/PROJECTS/DEREVERB3/timit_8k/reverb/test6/';
out_in = '/data/liyuy/PROJECTS/DEREVERB3/LSTM/exp5/output/output_wav/test';
path1 = sprintf('%s%d_wlen_%d_nfft_%d_overlap_%dbi_1024_',out_in,test_set,wlen,nfft,overlap);
...
path2 = sprintf('%s%d_1024/',path_in,test_set);
T60s = [0.3 0.6 0.9];
%i_loc = 1;
Fs = 8e3;
count = 1;
    ...
    for i = 1:500
        if test_set == 5
           ...
           if i ~= 1 && mod((i-1),125) == 0
            ...
            %est_file = sprintf('%sreverb_rir%04d_roomNum%d_t60%g_loc%d_8kHz.wav',path1,i,roomNum,T60,i);
            %est_file = sprintf('%sreverb_rir%04d_roomNum%d_t60%g_loc%d_%dkHz.wav',path1,i,roomNum,T60,i,Fs/1000);
